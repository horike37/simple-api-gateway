...
exports.action = function() {
  ...
  inquirer.prompt([
  {
    ...
    return inquirer.prompt([
             {
               type: 'confirm',
               name: 'isMappingTemplate',
               message: 'Set up mapping template?',
               default: false
             }
           ])
  
  }).then(function(answers) {
    if (answers.isMappingTemplate) {
      return inquirer.prompt([
               {
                 type: 'editor',
                 name: 'mappingTemplate',
                 message: 'Please edit mapping template',
                 default: '{}'
               }
      ]).then(function(answers){
        settings['mappingTemplate'] = answers['mappingTemplate']
      })
    }
  }).then(function() {
    return inquirer.prompt([
    ...
    return apigateway.putIntegrationResponse(params).promise()
      ...
      if (settings['enableCORS']) {
          let allowMethods = settings.method + ',OPTIONS'
          let params = {
              httpMethod: 'OPTIONS',
              resourceId: settings.resource.id,
              restApiId : settings.api_id,
